...
const renderActiveTask = () => {
  todoListArray
    .filter((item) => item.tdStatus === true)
    .forEach((item) => {
      document.getElementById(item.id).closest("li").style.display = "none";
    });
  todoListArray
    .filter((item) => item.tdStatus === false)
    .forEach((item) => {
...
const addTodoToList = (todoItem) => {
  ...
  checkBox.addEventListener(
    "click",
    changeCheckedStatus.bind(null, checkBox, todoItem)
  );

  let removeOne = newItem.querySelector("span");
  removeOne.addEventListener(
    "click",
    removeIndividualItem.bind(null, todoItem, newItem)
  );
...
const removeCompletedTodoHandler = (e) => {
  e.preventDefault();
...
formWrapper.addEventListener("onSubmit", formWrapHandler);
